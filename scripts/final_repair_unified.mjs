...
// Add final13
Object.keys(final13).forEach(id => {
  masterDataset[id] = final13[id];
});

// Add batch19 using idMap
Object.keys(idMap).forEach(id => {
  const scrapedSlug = idMap[id];
  if (batch19Raw[scrapedSlug]) {
    masterDataset[id] = batch19Raw[scrapedSlug];
  }
});

console.log(`Unified dataset created with ${Object.keys(masterDataset).length} products.`);

// 5. Build cleaned products.ts
// We'll iterate through all 32 IDs and replace their blocks in productsContent.
...
missingProducts.forEach(mp => {
    ...
    if (!descData) {
        console.warn(`[WARN] No data for ${id}`);
        return;
    }
    ...
    if (!match) {
        console.warn(`[WARN] ${id} not found in file`);
        return;
    }
    ...
    // Find the end of the object properly (next line starting with "  }," or "  }")
    let endIdx = productsContent.indexOf('\n  },', startIdx);
    if (endIdx === -1) endIdx = productsContent.indexOf('\n  }', startIdx);
    if (endIdx === -1) {
        console.error(`[ERRO] End not found for ${id}`);
        return;
    }
    endIdx += 1; // Include the newline/closing brace correctly
    ...
    // Replace the block in productsContent
    productsContent = productsContent.substring(0, startOfObject) + cleanBlock + productsContent.substring(endIdx + 2);
});

// 6. Final cleanup of potential double commas or formatting issues at the very end
productsContent = productsContent.replace(/,(\s*),/g, ','); 

fs.writeFileSync(productsPath, productsContent);
console.log('SUCCESS: products.ts repaired with 32 clean blocks.');
